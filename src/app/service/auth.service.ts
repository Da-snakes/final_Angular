...
/////////////////////////////////////////////////////////////////////////////////////////
  ...
  //Chef
  //Get all user from db WHERE nomRole="?"
  getUserFromDBByRoleNom(nomRle: string): Observable<User[]> {
    const url = `${this.apiURL2}/role/${nomRle}`;
...
isChefVIP(): Boolean {
  let adminVIP: boolean = false;
  if (!this.AdminOrUser) { return false; }
  if (this.AdminOrUser == "Chef") { adminVIP = true; }
  return adminVIP;
}
